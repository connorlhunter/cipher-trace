...
import { coverageUpdatedAt, parseLcov, renderCoverageReport } from "./render-coverage-report";
...
test("renders labeled coverage pages from TypeScript and Python LCOV", (): void => {
  ...
  const python = readFileSync(join(directory, "coverage", "python", "index.html"), "utf8");

  expect(index).toContain("Cipher Trace coverage");
  expect(index).toContain('href="python/index.html"');
  expect(index).toContain("requires at least 95% line and function coverage");
  expect(index).toContain('Updated <time datetime="2026-08-25T18:42:31.123Z">Aug 25, 2026</time>');
  ...
  expect(python).toContain("Generated from the FastAPI and shared-contract test suite.");
